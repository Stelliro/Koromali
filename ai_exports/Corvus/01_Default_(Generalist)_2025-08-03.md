...
- **100% Offline & Private:** Your conversations never leave your machine.
- **Voice Activated:** Use a wake word ("Corvus") to interact without touching the keyboard.
...
If you encounter any persistent errors or wish to perform a complete "factory reset," run the cleaner script for your OS. This will permanently delete the Python environment, all downloaded models, logs, and local configurations.
...
echo [Step 1/6] Deleting Python virtual environment ('venv')...
if exist "venv" ( rmdir /s /q venv )

echo [Step 2/6] Deleting all downloaded AI models...
if exist "llm_models" ( rmdir /s /q llm_models )
if exist "tts_models" ( rmdir /s /q tts_models )

echo [Step 3/6] Deleting all logs, settings, and temporary files...
del /q /f .env settings.json *.log crash.log pip_install.log output.wav >nul 2>nul

echo [Step 4/6] Deleting __pycache__ folders...
for /d /r . %%d in (__pycache__) do ( if exist "%%d" rmdir /s /q "%%d" )

echo [Step 5/6] Deleting old/obsolete root files from previous structures...
del /q /f assistant.py config.py logging_config.py main.py tui.css installer_tui.py app_settings.py >nul 2>nul

echo [Step 6/6] Deleting old 'assistant_app' source folder if it exists...
...
read -p "This action will perform a full factory reset of generated files. Are you sure? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    exit 1
fi
...
echo "[1/6] Deleting Python virtual environment ('venv')..."
rm -rf venv

echo "[2/6] Deleting all downloaded AI models..."
rm -rf llm_models tts_models

echo "[3/6] Deleting all logs, settings, and temporary files..."
rm -f .env settings.json *.log crash.log pip_install.log output.wav

echo "[4/6] Deleting __pycache__ folders..."
find . -type d -name "__pycache__" -exec rm -rf {} +

echo "[5/6] Deleting old/obsolete root files from previous structures..."
rm -f assistant.py config.py logging_config.py main.py tui.css installer_tui.py app_settings.py

echo "[6/6] Deleting old 'assistant_app' source folder if it exists..."
rm -rf assistant_app
...
### File: `/installer.log`
```log
2025-08-03 19:59:30,932 [INFO    ]  Virtual environment is missing or incomplete. Recreating...
2025-08-03 19:59:39,510 [INFO    ]  Virtual environment created successfully.
2025-08-03 19:59:39,511 [INFO    ]  Installing dependencies (this may take several minutes)...
2025-08-03 19:59:39,514 [INFO    ]  Running command: C:\Users\gike5\Desktop\AI_Python\Corvus\venv\Scripts\python.exe -m pip install -r C:\Users\gike5\Desktop\AI_Python\Corvus\requirements.txt
...
```py
# corvus_app/plugins/exit_command.py
...
```py
# corvus_app/plugins/registry.py
import importlib
import logging
from pathlib import Path
from .base import Command
...
class CommandRegistry:
    ...
    def __init__(self, plugins_dir: str = "corvus_app/plugins"):
...
```py
# corvus_app/plugins/time_command.py
from datetime import datetime
from .base import Command
from typing import TYPE_CHECKING
