...
This project is a fully interactive AI assistant for your terminal, powered by locally-run GGUF language models and a customizable TTS engine. It operates completely offline and privately, without relying on external services like Ollama.

## Features

-   **No External Dependencies:** Runs without requiring the Ollama service.
...
class Assistant:
    ...
    def reload(self):
        ...
        if active_model_path and Path(active_model_path).exists():
            ...
            self.llm_client = None
            self.is_running = True # App can run, but LLM is disabled
    ...
    def process_prompt(self, prompt: str) -> str:
        ...
        # Check for a direct command from the user's prompt
        ...
        # If no direct command, get a response from the LLM
        ...
        # Check if the LLM's response triggers a command
...
# This is a fallback for crashes that happen before the TUI is fully running
_console = Console()

# Set up logging as the very first step
try:
    # Ensure local model directories exist before setting up logging/app
    from pathlib import Path
    ...
    # If logging fails, we'll write to a crash log later
    _console.print("[bold red]FATAL: Failed to configure logging.[/bold red]")
    # Don't exit yet, let the main crash handler take over to generate the log.
    pass
...
def main():
    ...
    try:
        ...
        # This catches any unhandled exception during the application's lifecycle.
        
        # 1. Log the crash using the configured logger if it's available.
        logger.critical("A critical unhandled error occurred in main. The application will now terminate.", exc_info=True)
        
        # 2. Create a user-friendly, standalone crash.log file.
        ...
        # 3. Inform the user in the console.
        ...
        _console.print(f"[yellow]The application will now close.[/yellow]\n")
...
if not exist "venv\deps_installed.marker" (
    echo [INFO] Installing required Python packages. This may take several minutes...
    echo [INFO] Note: A C++ compiler is required for the llama-cpp-python package.
    pip install --quiet -r "requirements.txt"
    if %errorlevel% neq 0 (
      echo [ERROR] Failed to install dependencies. Please check your C++ compiler setup.
      pause
      exit /b 1
    )
...
# Use a marker file to avoid reinstalling dependencies every run
if [ ! -f "venv/deps_installed.marker" ]; then
    print_info "Installing required Python packages. This may take several minutes..."
