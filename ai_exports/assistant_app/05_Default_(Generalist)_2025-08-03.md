        ...
        if self.assistant.is_running:
            ...
            list_view.append(
                ConversationListItem(
                    "Assistant failed to initialize. Check active models and logs.", "assistant"
                )
            )
    ...
    def listen_for_wake_word(self):
        ...
        while self.input_mode == "voice" and self.assistant.is_running:
            ...
            if config.WAKE_WORD in text:
                ...
                return # Important: exit the listen loop once a command is handled
...
class DownloadScreen(Screen):
    ...
    def on_mount(self) -> None:
        self.run_worker(self.download_model_file, thread=True)
...
class TTSDownloadScreen(Screen):
    ...
    def on_mount(self) -> None:
        self.run_worker(self.download_tts_model, thread=True)
...
class InstallerApp(App):
    ...
    def on_mount(self) -> None:
        ...
        self.run_worker(self.run_installation_orchestrator, thread=True, exclusive=True)

    def update_status(self, message: str) -> None:
        self.call_from_thread(self.query_one("#status-line", Static).update, f"Status: {message}")
        logger.info(message)

    def run_installation_orchestrator(self):
        """Coordinates the main installation steps."""
        logger.info("=" * 60)
        logger.info(f"Installation orchestrator started at {datetime.now()}")
        logger.info("=" * 60)
        
        try:
            if MARKER_FILE.exists():
                self.update_status("Dependencies are already installed.")
                self.run_worker(self.launch_main_app, thread=True, exclusive=True)
                return

            self.create_venv()

        except Exception as e:
            self.update_status("[bold red]An unexpected error occurred![/]")
    ...
    def install_dependencies(self):
        ...
        self.call_from_thread(progress_bar.update, total=None)
    ...
    def launch_main_app(self):
        """Launches the main assistant in a new process."""
        self.update_status("All checks passed. Launching application...")
        # CORRECTED: Use standard Python `time.sleep` in a worker thread, not self.app.sleep.
        time.sleep(2)
...
class InstallerApp(App):
    ...
    def create_venv(self):
        ...
        # Whether the venv was found or just created, proceed to install dependencies
        self.install_dependencies()
    ...
    def install_dependencies(self):
        ...
        progress_bar = self.query_one(ProgressBar)
        self.call_from_thread(progress_bar.add_class, "visible")
        self.call_from_thread(progress_bar.update, total=None)  # Indeterminate progress
        ...
        if process.returncode == 0:
            self.call_from_thread(progress_bar.update, total=100, progress=100) # Mark as complete
            self.update_status("Dependencies installed successfully!")
            MARKER_FILE.touch()
            self.create_env_file()
        else:
    ...
    def launch_main_app(self):
        """Launches the main assistant in a new process."""
        self.update_status("All checks passed. Launching application...")
        # CORRECTED: Use standard Python `time.sleep` in a worker thread.
        time.sleep(2)
...
class ConversationScreen(Screen):
    ...
    def listen_for_wake_word(self):
        """Continuously listens for the wake word. Runs in a thread."""
        self.app.post_message(StatusUpdate("LISTENING..."))
        logger.debug("Starting wake word listener.")
        ...
        while self.input_mode == "voice" and self.assistant.is_running:
            ...
            if config.WAKE_WORD in text:
                ...
                if prompt:
                    ...
                    self.app.post_message(StatusUpdate("Yes?"))
                    self.run_worker(self.speak_and_reset_status, "Yes?", thread=True, exclusive=True, group="audio_output")
                return # Exit this worker, a new one will be spawned after TTS

    def process_prompt(self, prompt: str):
        """Processes the prompt and gets a response. Runs in a thread."""
        response = self.assistant.process_prompt(prompt)
        self.post_message(AssistantResponse(response, prompt))
        if not self.assistant.is_running:
    ...
    def speak_and_reset_status(self, text: str):
        """Synthesizes speech and restarts the listener. Runs in a thread."""
        speak_text(text)
        if self.input_mode == "voice":
            # Start a new listener worker.
            self.run_worker(self.listen_for_wake_word, thread=True, exclusive=True, group="audio_input")
        else:
    ...
    def action_toggle_input_mode(self) -> None:
        ...
        if self.input_mode == "voice":
            ...
            text_input.disabled = False
            text_input.focus()
            # Stop any running audio listeners.
            self.app.cancel_workers(group="audio_input")
            self.post_message(StatusUpdate("TYPING..."))
        else:
            self.input_mode = "voice"
